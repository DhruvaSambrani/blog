...
if [[ $PAGES == "1" ]]; then
    ...
    zk list -x index.md --format '{{metadata.updated}}\t{{format-date created "%Y-%m-%d"}}\t{{title}}\t{{path}}' 2>/dev/null |
        ...
        PAGEROOT="$pageroot" pandoc -t html5 --template=templates/contents.html -V "pageroot=$pageroot" -s -o build/contents.html --lua-filter=filters/html_links.lua --wrap=preserve
    ...
    awk -v r="$RECENT" '{gsub("{{recent}}", r); print}' index.md |
        PAGEROOT="$pageroot" pandoc -t html5 --template=templates/home.html -V "pageroot=$pageroot" -s -o build/index.html --lua-filter=filters/html_links.lua --wrap=preserve
